...
BOOST_AUTO_TEST_CASE(SimpleQuerySingleBlobTest)
{
    auto data = odbcx::query_one<std::vector<std::uint8_t>>(dbc, "SELECT pb FROM test where messagetype = 'message 1'");
    BOOST_CHECK_EQUAL(!data, false);
    BOOST_CHECK_NE((*data).size(), 0);
}

BOOST_AUTO_TEST_CASE(SimpleQuerySingleBlobDataReadTest)
{
    auto stmt = odbcx::query(dbc, "SELECT pb FROM test where messagetype = 'message 1'");
    SQLULEN fetched = 0;
